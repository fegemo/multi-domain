...
class StarGANModelTest(tf.test.TestCase):
    def skip___test_concat_image_and_domain(self):
        ...
        number_of_domains = 2
        config = OptionParser().parse(
            ["stargan-unpaired", "--image-size", str(image_size), "--rmxp", "--no-aug", "--batch", str(batch_size)])
        ...
        self.assertAllEqual(
            tf.constant([
                [[[1, 1, 1, 0, 0, 0, 0, 1], [2, 2, 2, 0, 0, 0, 0, 1]],
                 [[3, 3, 3, 0, 0, 0, 0, 1], [4, 4, 4, 0, 0, 0, 0, 1]]],
                [[[10, 10, 10, 0, 0, 1, 0, 0], [20, 20, 20, 0, 0, 1, 0, 0]],
                 [[30, 30, 30, 0, 0, 1, 0, 0], [40, 40, 40, 0, 0, 1, 0, 0]]],
                [[[100, 100, 100, 0, 0, 1, 0, 0], [200, 200, 200, 0, 0, 1, 0, 0]],
                 [[300, 300, 300, 0, 0, 1, 0, 0], [400, 400, 400, 0, 0, 1, 0, 0]]]
            ]),
            image_and_domain
        )
...
class NParamsSupplierTest(tf.test.TestCase):
    def test_call(self):
        params = [1, 2, 3]
        three_supplier = NParamsSupplier(3)
        two_supplier = NParamsSupplier(2)
        one_supplier = NParamsSupplier(1)

        self.assertLen(three_supplier(*params), 3)
        self.assertLen(two_supplier(*params), 2)
        self.assertLen(one_supplier(*params), 1)

    def setUp(self):
        super(NParamsSupplierTest, self).setUp()
