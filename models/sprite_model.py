...
class SpriteEditorModel(RemicModel):
    ...
    def discriminator_loss(self, predicted_patches_real, predicted_patches_fake, r1_penalty, r2_penalty):
        ...
        total_loss = [super_losses["adversarial"][d] +
                      lambda_regularization * super_losses["l2-regularization"][d] +
                      super_losses["gradient-penalty"][d] for d in range(number_of_domains)]

        return {
            "adversarial": super_losses["adversarial"],
            "real": super_losses["real"],
            "fake": super_losses["fake"],
            "l2-regularization": super_losses["l2-regularization"],
            "gradient-penalty": super_losses["gradient-penalty"],
            "total": total_loss
        }

    def generator_loss(self, fake_extracted_predicted_patches, fake_random_predicted_patches, fake_cyclic_predicted_patches, real_predicted_patches, 
                       fake_cohesion_evaluation, generated_images, target_images, cyclically_generated_images, input_dropout_mask, 
                       ec_mean, ec_log_var, random_codes, recovered_codes_mean, target_palettes, 
                       temperature, batch_shape):
        """
        Calculates the generator loss for the Sprite Editor model.
        :param fake_extracted_predicted_patches: discriminator outputs for the generated images from extracted codes. 
            The shape depends on the number of domains and discriminator scales: [d, ds] x [hb, ?, ?, 1] where 
            d is the number of domains, ds is the number of discriminator scales, b is the batch size, and 
            ? is the size of discriminated patches.
        :param fake_random_predicted_patches: discriminator outputs for the generated images from random codes. The shape
            is similar to the fake_extracted_predicted_patches.
        :param fake_cyclic_predicted_patches: discriminator outputs for the cyclically generated images from random codes.
            The shape is similar to the fake_extracted_predicted_patches.
        :param real_predicted_patches: discriminator outputs for the real images. The shape is [d, ds] x [hb, ?, ?, 1].
        ...
        :param generated_images: the fake images generated by the generator. The shape depends on the number of
            ...
            The first half of the batch is generated with the extracted codes, and the second half with the random codes.
        :param target_images: the ground truth images for the generator. The shape depends on the number of
            generator scales: [gs] x [b, d, ?, ?, c].
        :param cyclically_generated_images: the images generated from the generated images with random codes, used for the
            cyclic consistency loss. The shape is [gs] x [hb, d, ?, ?, c].
        ...
        adversarial_extracted_loss = self.adv_loss.calculate_generator_loss(fake_extracted_predicted_patches, real_predicted_patches)
        adversarial_random_loss = self.adv_loss.calculate_generator_loss(fake_random_predicted_patches, real_predicted_patches)
        adversarial_cyclic_loss = self.adv_loss.calculate_generator_loss(fake_cyclic_predicted_patches, real_predicted_patches)
        adversarial_extracted_loss = tf.reduce_mean(adversarial_extracted_loss)
        adversarial_random_loss = tf.reduce_mean(adversarial_random_loss)
        adversarial_cyclic_loss = tf.reduce_mean(adversarial_cyclic_loss)
        adversarial_loss = (adversarial_extracted_loss + adversarial_random_loss + adversarial_cyclic_loss) / 3.0
        # adversarial_loss (shape=[])

        # 2. calculates reconstruction (l1) loss between the target and the generated images using extracted codes
        reconstruction_loss = [tf.reduce_mean(tf.abs(target_images[gs][:half_batch_size] - generated_images[gs][:half_batch_size]))
                               for gs in range(generator_scales)]
        reconstruction_loss = tf.reduce_mean(reconstruction_loss)

        # 3. calculates the l1 cyclic loss between the original images and the cyclically generated images 
        # that go through the random codes path
        cyclic_loss = [tf.reduce_mean(tf.abs(target_images[gs][half_batch_size:] - cyclically_generated_images[gs]))
                       for gs in range(generator_scales)]
        cyclic_loss = tf.reduce_mean(cyclic_loss)
        ...
        # seploss (how I think the networks should be penalized)
        total_g_loss = self.lambda_adversarial * adversarial_loss + \
                     self.lambda_reconstruction * reconstruction_loss + \
                     self.lambda_cyclic_reconstruction * cyclic_loss + \
        ...
        total_e_loss = self.lambda_latent_reconstruction * latent_reconstruction_loss + \
                        self.lambda_kl * kl_loss
        # CORRECTloss (according to my best attempt at reverse engineering bicyclegan)
        # total_g_loss = self.lambda_adversarial * adversarial_loss + \
        #              self.lambda_reconstruction * reconstruction_loss + \
        #              self.lambda_cyclic_reconstruction * cyclic_loss + \
        #              self.lambda_latent_reconstruction * latent_reconstruction_loss + \
        #              self.lambda_cohesion * cohesion_loss + \
        #              self.lambda_palette * palette_coverage_loss
        # total_e_loss = self.lambda_adversarial * adversarial_extracted_loss + \
        #                 self.lambda_reconstruction * reconstruction_loss + \
        #                 self.lambda_kl * kl_loss

        # returns the weighted total generator loss and the individual terms
        return {"adversarial": adversarial_loss, "reconstruction": reconstruction_loss, "cyclic": cyclic_loss,
                "latent-reconstruction": latent_reconstruction_loss, "palette-coverage": palette_coverage_loss,
                "kl": kl_loss,
                "cohesion": cohesion_loss,
                "total-g": total_g_loss,
                "total-e": total_e_loss}
    ...
    def train_step(self, batch, step, evaluate_steps, t):
        ...
        for s in range(self.config.generator_scales):
            ...
            target_image = tf.reshape(target_image, (batch_size * number_of_domains,
        ...
        # real_images ([generator_scales x shape=[b, d, ?, ?, c]])
        ...
        with (tf.GradientTape(persistent=True) as gen_tape):
            ...
            # A.2-3. generates images using the extracted codes [:half_batch_size], then with random codes [half_batch_size:]
            ...
            # A.5. assembles a new input from the generated images with random codes and the original input with empty mask
            # this should work as an unsupervised cyclic loss to the ground truth when going to the random code path
            ...
            # cyclically_generated_images ([generator_scales x shape=[hb, d, ?, ?, c]])

            # A.6. discriminates the full size generated images that used the extracted and random codes, concatenated
            generated_images_full_size = tf.concat([generated_images_with_extracted_codes[0],
                                                    generated_images_with_random_codes[0],
                                                    cyclically_generated_images[0]], axis=0)
            ...
            real_predicted = [listify(self.discriminators[d](source_images[:half_batch_size, d], training=True))
                              for d in range(number_of_domains)]
            ...
            # A.7. calculates the generator loss
            g_loss = self.generator_loss(
                fake_predicted_extracted, fake_predicted_random, fake_predicted_cyclic, real_predicted,
                fake_cohesion_evaluation,
                generated_images, real_images, cyclically_generated_images,
                input_keep_mask,
        ...
        # A.8. applies the generator and encoder gradients
        generator_gradients = gen_tape.gradient(g_loss["total-g"], self.generator.trainable_variables)
        self.generator_optimizer.apply_gradients(zip(generator_gradients, self.generator.trainable_variables))

        diversity_encoder_gradients = gen_tape.gradient(g_loss["total-e"],
        ...
        with tf.GradientTape(persistent=True) as disc_tape:
            ...
            for i, disc in enumerate(self.discriminators):
                ...
                r1_penalty += [self.gradient_penalty(disc, batch[i])]
                r2_penalty += [self.gradient_penalty(disc, generated_images[:, target_domain])]
        ...
        g_loss_summaries = {
            "total_g_loss": g_loss["total-g"],
            "total_e_loss": g_loss["total-e"],
            "adversarial_loss": g_loss["adversarial"],
            "reconstruction_loss": g_loss["reconstruction"],
            "cyclic_loss": g_loss["cyclic"],
    ...
    def mix_and_discriminate(self, discriminator, real_image, fake_image, half_batch_size):
        ...
        :param real_image: tensor with real images, shape=[b, s, s, c]
        :param fake_image: tensor with fake images, shape=[b, s, s, c]
        ...
        :return: discriminated patches for real and fake images. As there are 3 discriminator scales, the output
            has shape [ds] x [b, ?, ?, 1] where ds is the number of discriminator scales,
        """
        ...
        predicted_patches = listify(discriminator(discriminator_input, training=True))
        ...
        # real_predicted_patches ([ds] x shape=[b, ?, ?, 1])
        # fake_predicted_patches ([ds] x shape=[b, ?, ?, 1])
        return real_predicted_patches, fake_predicted_patches

    def split_fake_predictions(self, fake_predicted):
        number_of_domains = self.config.number_of_domains
        fake_predicted_extracted = [[] for _ in range(number_of_domains)]
        fake_predicted_random = [[] for _ in range(number_of_domains)]
        fake_predicted_cyclic = [[] for _ in range(number_of_domains)]
        
        for d in range(number_of_domains):
            for scale_pred in fake_predicted[d]:
                # Split into three equal parts
                split_parts = tf.split(scale_pred, num_or_size_splits=3, axis=0)
                
                fake_predicted_extracted[d].append(split_parts[0])
                fake_predicted_random[d].append(split_parts[1])
                fake_predicted_cyclic[d].append(split_parts[2])
        
        return fake_predicted_extracted, fake_predicted_random, fake_predicted_cyclic
    ...
    def select_examples_for_visualization(self, train_ds, test_ds):
        ...
        if is_trained_to_inpaint:
            ...
            example_masks = [create_random_inpaint_mask(tf.stack(examples[ex][0])[tf.newaxis, ...], holes) for ex, holes
                             in
    ...
    def debug_discriminator_output(self, batch, image_path):
        ...
        real_patches = [listify(self.discriminators[target_domains[i]](real_images[i][tf.newaxis, ...]))
                        for i in range(batch_size)]
        fake_patches = [
            listify(self.discriminators[target_domains[i]](fake_images[i][0][0][target_domains[i]][tf.newaxis, ...]))
            for i in range(batch_size)]
        # if d_scales == 1:
