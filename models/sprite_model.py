...
class SpriteEditorModel(RemicModel):
    ...
    def generator_loss(self, fake_predicted_patches, real_predicted_patches, generated_images, target_images,
        ...
        adversarial_loss = self.adv_loss.calculate_generator_loss(fake_predicted_patches, real_predicted_patches)
    ...
    def train_step(self, batch, step, evaluate_steps, t):
        ...
        with (tf.GradientTape(persistent=True) as gen_tape):
            ...
            real_predicted = [listify(self.discriminators[d](source_images[:, d], training=True))
                              for d in range(number_of_domains)]
...
class R3GANResidualBlock(layers.Layer):
    ...
    def __init__(self, input_channels, variance_scaling_parameter):
        ...
        self.kernel_initializer = keras_utils.MSRInitializer(activation_gain)
        self.linear1 = None
        self.linear2 = None
        self.linear3 = None
    ...
    def build(self, input_shape):
        ...
        if input_channels != self.output_channels:
            self.linear = layers.Conv2D(self.output_channels, 1, strides=1, padding="same",
                                        use_bias=False, kernel_initializer=keras_utils.MSRInitializer(1.0))
    ...
    def build(self, input_shape):
        ...
        if input_channels != self.output_channels:
            self.linear = layers.Conv2D(self.output_channels, 1, strides=1, padding="same", use_bias=False,
                                        kernel_initializer=keras_utils.MSRInitializer(1.0))
...
def build_r3gan_generator(config):
    """
    ...
    initial_stage = DownsampleStage(channels * domains + 1 + domains, 384, image_size, variance_scaling)
    x = initial_stage(encoder_input)
    x = DownsampleStage(768, 768, image_size // 2, variance_scaling)(x)
    x = DownsampleStage(768, 768, image_size // 4, variance_scaling)(x)
    x = DownsampleStage(768, 768, image_size // 8, variance_scaling)(x)
    x = DownsampleStage(768, 768, image_size // 16, variance_scaling)(x)
    x = DownsampleStage(768, 768, image_size // 32, variance_scaling)(x)

    bottleneck = R3GANResidualBlock(768, variance_scaling)(x)

    x = UpsampleStage(768, 768, image_size // 64, variance_scaling)(bottleneck)
    x = UpsampleStage(768, 768, image_size // 32, variance_scaling)(x)
    x = UpsampleStage(768, 768, image_size // 16, variance_scaling)(x)
    x = UpsampleStage(768, 768, image_size // 8, variance_scaling)(x)
    x = UpsampleStage(768, 768, image_size // 4, variance_scaling)(x)
    x = UpsampleStage(768, 384, image_size // 2, variance_scaling)(x)
