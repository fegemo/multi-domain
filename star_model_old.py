...
from side2side_model_old import S2SModel
from networks_old import StarGANGenerator, StarGANDiscriminator
import io_utils_old as io_utils
...
class StarGANModel(S2SModel):
    ...
    def select_real_and_fake_images_for_fid(self, num_images, dataset):
        real_images = np.ndarray((num_images, 64, 64, 4))
        fake_images = np.ndarray((num_images, 64, 64, 4))
    ...
    def generate_comparison(self, examples, save_name=None, step=None):
        ...
        for i, batch in enumerate(examples):
            ...
            target_domain_name = DOMAIN_FOLDERS[
                target_domain_side_index]  # tf.gather(DOMAIN_FOLDERS, target_domain_side_index)
...
class CollaGANModelOld(PairedStarGANModel):
